...
class getAngler : public Summation<Line,Angler>
{
    ...
    protected:
        ...
        Angler neutral() const override {return Angler("",false,0);}
...
class getAnwer : public MaxSearch<Angler,int>
{
    public:
        ...
        bool cond (const Angler&e) const override {return e.iscaught == false;} 
};
...
int main()
{
    try
    {
        ...
        if(ga.found())
        {
            if(ga.optElem().num != 0)
            {
                cout<< ga.optElem().name << " attended " <<ga.optElem().num<<" championships but fished nothing"<<endl;
            }else
            {
